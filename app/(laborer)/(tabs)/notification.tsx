...
export default function NotificationScreen() {
  ...
  const updateUnreadCount = async (list: Notification[]) => {
    try {
      ...
      await AsyncStorage.setItem('laborerUnreadCount', String(unread));
    } catch (e) {}
  };
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'verification_submission': return 'document-text';
      case 'verification_update': return 'shield-checkmark';
      case 'job_request': return 'briefcase';
  ...
  const handleOpenNotification = async (item: Notification) => {
    ...
    if (item.type === 'job_request' && item.data?.bookingId) {
      router.push({ pathname: '/(laborer)/job-details/[id]', params: { id: item.data.bookingId } });
    }
  };
