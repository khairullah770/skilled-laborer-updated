...
export const fetchCustomers = async () => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to fetch customers: ${response.status} ${response.statusText} - ${text}`,
    );
  }
  return response.json();
};
...
export const laborerAccountAction = async (
  id: string,
  action: "warning" | "temp_block" | "perm_block" | "unblock",
  reason: string,
) => {
  const response = await fetch(`${API_URL}/users/${id}/account-action`, {
    method: "PUT",
    headers: getHeaders(),
  ...
  if (!response.ok) {
    ...
    throw new Error(err.message || "Failed to perform account action");
  }
  return response.json();
};
